...
int main(int argc, char * argv[])
{
    ...
    if(!locxparser) { /* Checking for empty string */
        ...
        if(strcmp(tmpl_directory,".") != 0) {
            printf("\n***Environment variable FLAME_XPARSER_DIR not set - looking in %s for Templates\n",tmpl_directory);
        } else {
            printf("\n***Environment variable FLAME_XPARSER_DIR not set - looking in current directory for Templates\n");
        }
    } else {
